...
const SUPABASE_URL = process.env.NGSRV_SUPABASE_URL ||
...
function toolsForList(tools, lean) {
    if (!lean)
        return tools;
...
export const TOOL_DEFINITIONS = [
    {
        name: "tunnel.start",
        title: "Start tunnel",
        description: "Expose a local HTTP server on a public HTTPS URL via ngsrv. " +
        ...
        inputSchema: {
            ...
            properties: {
                port: {
                    type: "number",
                    description: "TCP port your local HTTP server listens on. Example: 3000 for Next.js, 8080 for APIs, 4242 for Stripe CLI.",
                    minimum: 1,
                    maximum: 65535,
                },
            },
    ...
    },
    {
        name: "tunnel.list",
        title: "List tunnels",
        description: "List HTTPS tunnels started by this MCP process in the current session. " +
        ...
        inputSchema: {
            ...
            properties: {
                limit: {
                    type: "number",
                    description: "Maximum number of tunnels to return. Omit to return all active tunnels in this process.",
                    minimum: 1,
                },
            },
    ...
    },
    {
        name: "tunnel.stop",
        title: "Stop tunnel",
        description: "Stop an ngsrv tunnel started by this MCP session. " +
        ...
        inputSchema: {
            ...
            properties: {
                port: {
                    type: "number",
                    description: "Local port of the tunnel to stop. Must match a port from tunnel.list. Example: 3000.",
                    minimum: 1,
                    maximum: 65535,
                },
            },
    ...
    },
    {
        ...
        description: "Obtain a 12-hour ngsrv agent token for tunnel commands. " +
        ...
        inputSchema: {
            ...
            properties: {
                refresh: {
                    type: "boolean",
                    description: "When true, request a new trial token even if NGSRV_API_TOKEN is already set.",
                },
            },
            additionalProperties: false,
        },
    ...
    },
    {
        name: "docs.install",
        title: "Install help",
        description: "Return ngsrv CLI install commands and MCP client configuration snippets. " +
        ...
        inputSchema: {
            ...
            properties: {
                client: {
                    type: "string",
                    description: "MCP client to tailor config for. Example: cursor, claude, windsurf.",
                },
            },
            additionalProperties: false,
        },
 ...
 */
export function createNgsrvMcpServer(options = {}) {
    const leanToolsList = options.leanToolsList ?? true;
    const server = new Server({ name: "ngsrv", version: "1.0.3" }, {
        ...
        prompts: [
            {
                ...
                description: "Step-by-step prompt to expose a local dev server (default port 3000) on a public HTTPS URL.",
                arguments: [
                    {
                        name: "port",
            ...
            },
            {
                name: "docs.setup",
                title: "MCP setup",
                description: "Shows how to add ngsrv to Cursor or Claude Desktop using npx and optional API token.",
                arguments: [],
            },
        ],
    }));
                ...
                messages: [
                    {
                        ...
                        content: {
                            type: "text",
                            text: `Expose my local HTTP server on port ${port} with ngsrv.\n` +
                ...
                messages: [
                    {
                        ...
                        content: {
                            type: "text",
                            text: "Show ngsrv MCP setup: npx -y ngsrv-mcp, optional NGSRV_API_TOKEN, " +
        ...
        resources: [
            {
                ...
                description: "Install commands and links for ngsrv CLI, npx @ngsrv/cli, and ngsrv-mcp.",
                mimeType: "text/plain",
            },
        ],
    }));
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            ...
            if (tool === "tunnel.start") {
                ...
                const token = await ensureToken();
                const child = spawn(ngsrvBin(), ["http", String(port), "--json", "--no-tui", "-q"], {
